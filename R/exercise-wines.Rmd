...
It is totally fine if you only managed to worked on the first few exercises.
...
The interpretation methods in the `iml` package require that the machine learning model and the data is wrapped in a Predictor object.
...
However, at around alcohol level of 12, the wine quality does not seem to improve much.
</details>
```{r, eval = !show.solution, echo = FALSE, results='asis'}
cat("-->")
```
...
Plot the PDP and ICE curves. What do you observe?
...
Compute the interaction strength using the H-statistic of all features to identify the feature that contains the strongest interactions with other features.
...
The previous exercise, we only identified the feature with strongest interactions.
...
Compute a 2D-PDP between alcohol and one of the most important feature that interacts with alcohol (from the previous exercise).

`r colorize("hint", "blue")`: In previous exercises, we used `FeatureEffect$new` to create 1 dimensional PDPs. But now you are asked to create a 2 dimensional PDP for alcohol and the most important feature that interacts with alcohol from the previous exercise. Look up the help page `?FeatureEffect` to find out how to do this.
...
Shapley values are (like Counterfactuals) a local IML method, meaning that they provide explanations for one specific observation. 
However, compared to counterfactuals, they explain how much each feature attributed to the prediction of the regarded observation.
...
This exercise provides the fundamental ideas of another IML approach which is called *Counterfactual Explanations*. 
...
`r colorize("hint", "blue")`: After having chosen a wine with quality 5, you need to manually change feature values of 1-2 features of this wine until its predicted wine quality is higher than 6 (while holding all other features unchanged). To do so, you may create a sequence of feature values using the `seq` function and may use the `expand.grid` function to obtain a grid containing all possible combinations multiple sequences, e.g.,
```{r}
